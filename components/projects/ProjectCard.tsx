...
export function ProjectCard({ project, index = 0 }: { project: Project; index?: number }) {
  ...
  return (
    ...
    >
      <div className="group rounded-2xl border border-border-soft bg-bg-elevated p-6 sm:p-7 hover:border-accent/40 transition-colors h-full flex flex-col">
        <Link href={`/projects/${project.slug}`} className="block">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-text-faint font-mono-tag mt-1">{project.category} · {project.duration}</p>
            </div>
        ...
        {(project.liveDemo || project.github) && (
          <div className="mt-5 pt-4 border-t border-border-soft flex flex-wrap gap-3">
            {project.liveDemo && (
              <a
            ...
            {project.github && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-1.5 text-xs font-medium text-text-muted hover:text-text hover:underline"
              >
